...
exports.genre_delete_post = (req, res, next) => {
  async.parallel({
    ...
    if (results.genres_books.length > 0) {
      // Genre has associated books still. Re-render form in same way as for GET route
      res.render('genre_delete', { title: 'Delete Genre', genre: results.genre, genre_books: results.genres_books });
    }

    // Genre has no associated books. Delete object and redirect to list of genres
...
exports.genre_update_get = (req, res, next) => {
  Genre.findById(req.params.id, (err, genre) => {
    if (err) { return next(err); }
    if (genre === null) {
      // No matching genre record exists
      let err = new Error('Genre not found');
      err.status = 404;
      return next(err);
    }
