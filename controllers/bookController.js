...
exports.book_delete_get = (req, res, next) => {
  async.parallel({
    ...
    if (err) { return next(err); }
    if (results.book === null) {
      // No matching book record exists
...
exports.book_delete_post = (req, res, next) => {
  async.parallel({
    ...
    if (results.book_instances.length > 0) {
      // Book instances still exist. Re-render in same way as for GET route
      res.render('book_delete', { title: 'Delete Book', book: results.book, book_instances: results.book_instances });
    }
    // No book instances exist for current book. Delete object and redirect to list of books
    Book.findByIdAndRemove(req.body.bookid, (err) => {
      if (err) { return next(err); }
      // Success - got to list of books
      res.redirect('/catalog/books');
    });
  });
};
